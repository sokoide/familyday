...
`make run` で起動するサーバは、Go の標準 `net/http` を使った単一プロセスの HTTP サーバです。`10人前後の同時接続` なら、通常のプレイ速度では概ね問題なく動きます。ただし、次の制約があります。

- **判定とエンディングは Gemini API 依存**: `/api/judge` と `/api/ending` は各リクエストごとに外部APIを呼ぶため、同時アクセスが増えるとレイテンシは Gemini 側の応答時間に左右されます。
- **レート制限はセッション単位**: `/api/judge` は `30回/分`、`/api/ending` は `5回/分` のメモリベース制限です。別端末・別セッションなら基本的に独立します。
- **レートリミッタはメモリ内**: 複数プロセス構成では共有されません。`make run` の単一プロセス運用向けです。
- **エンディング保存はローカルFS**: 1件ごとに JSON を保存します。10人規模なら軽いですが、永続化先が遅いストレージだと体感に影響します。
- **リクエストはサイズ制限あり**: `POST /api/judge` と `POST /api/ending` はリクエスト本文を 64KiB に制限しています。

要するに、`10人程度の同時利用` は想定内ですが、`大量連打` や `Gemini API の遅延・失敗` があると待ち時間は伸びます。会場利用では、各端末が独立して1プレイずつ進める前提で使ってください。
